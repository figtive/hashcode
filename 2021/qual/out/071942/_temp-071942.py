    ...
    for intersection in intersections:
        ...
        for street in intersection.streets_in:
            if streets_popularity[street.name] >= 0 or intersection_popularity[intersection.id] > 0:
                ...
                duration = (1 * ((streets_popularity[street.name] if streets_popularity[street.name] > 1 else 1) / intersection_popularity[intersection.id])) * (D // constant)
                if duration < 1:
                    duration = 1
                if duration > D:
                    duration = D
