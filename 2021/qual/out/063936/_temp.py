    ...
    for intersection in intersections:
        ...
        for street in intersection.streets_in:
            if streets_popularity[street.name] > 0 or intersection_popularity[intersection.id] > 0:
                ...
                constant = 2 if D < 10 else random.choice([100,200,300,400,1000])
                duration = streets_popularity[street.name] * ((intersection_street_length[intersection.id])) / intersection_popularity[intersection.id] * D // constant
                if duration == 0:
                    duration = 1
