    ...
    for intersection in intersections:
        ...
        for street in intersection.streets_in:
            if streets_popularity[street.name] > 0 or intersection_popularity[intersection.id] > 0:
                ...
                constant = 2 if D < 10 else random.choice([500,1000,1500])
                max_route = 1 if len(street.cars) == 0 else len(street.cars[0].streets)
                for car in street.cars:
                    max_route = len(car.streets) if max_route < len(car.streets) 
                duration = (1/max_route)*streets_popularity[street.name] * ((street.L / intersection_street_length[intersection.id])) / intersection_popularity[intersection.id] * D // constant
                if duration == 0:
                    duration = 1
