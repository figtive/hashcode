    ...
    for intersection in intersections:
        ...
        for street in intersection.streets_in:
            if streets_popularity[street.name] > 0 or intersection_popularity[intersection.id] > 0:
                ...
                constant = 2 if D < 10 else 1500
                # duration = (2 * (streets_popularity[street.name] / intersection_popularity[intersection.id])) * (1 * (1 + (street.L / intersection_street_length[intersection.id]))) * (D // constant)
