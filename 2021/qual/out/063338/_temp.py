    ...
    for intersection in intersections:
        ...
        for street in intersection.streets_in:
            if streets_popularity[street.name] > 0 or intersection_popularity[intersection.id] > 0:
                ...
                constant = 2 if D < 10 else random.randint(1000,3000)
                duration = streets_popularity[street.name] * ((street.L / intersection_street_length[intersection.id]))  * D // constant
                if duration == 0:
                    duration = 1
