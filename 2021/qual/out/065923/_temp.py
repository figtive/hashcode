    ...
    for intersection in intersections:
        ...
        for street in intersection.streets_in:
            if streets_popularity[street.name] > 0 or intersection_popularity[intersection.id] > 0:
                ...
                duration = (1.2 * (streets_popularity[street.name] / intersection_popularity[intersection.id])) * (0.5 * (1 + (street.L / intersection_street_length[intersection.id]))) * (D // constant)
                if duration < 1:
                    duration = 1
                if duration > D:
                    duration = D
