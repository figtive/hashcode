...
def main():
    ...
    D, intersection_c, street_c, car_c, F = int(D), int(
        intersection_c), int(street_c), int(car_c), int(F)
    intersections = []
    streets = {}
    cars = []
    streets_popularity = {}
    ...
    for intersection in intersections:
        ...
        for street in intersection.streets_in:
            if streets_popularity[street.name] > 0 or intersection_popularity[intersection.id] > 0:
                ...
                constant = 2 if D < 10 else 200
                # duration = (1 * (streets_popularity[street.name] / intersection_popularity[intersection.id])) * (1 * (1 + (street.L / intersection_street_length[intersection.id]))) * (D // constant)
                duration = (
                    ((streets_popularity[street.name]) / intersection_popularity[intersection.id])) * (D // constant)
                if duration < 1:
                    duration = 1
                if duration > D:
                    duration = D
        ...
        schedule.sort(
            key=lambda s: start_streets_popularity[s[0]], reverse=True)
    ...
    def step(self, T, D, F):
        if self.Lremaining == 0:
            ...
            if self.current_street.end.green[0] == self.current_street.name:
                ...
                # @intersection, move INTO next street @next step
                self.Lremaining = self.current_street.L
